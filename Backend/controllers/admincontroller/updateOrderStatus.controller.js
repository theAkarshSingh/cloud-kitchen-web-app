...
import { sendOrderStatusUpdate } from "../../services/notification.service.js";
...
const updateOrderStatus = asyncHandler(async (req, res) => {
  ...
  if (orderStatus === "delivered") {
    ...
    // Only mark COD orders as paid on delivery; online/UPI payment is verified separately
  ...
  User.findById(order.user)
    .then((user) => {
      if (user) sendOrderStatusUpdate(user.email, order).catch(() => {});
    })
    .catch(() => {});
